...
export const FetchOrders = () => {
  return async (dispatch) => {
    try {
      ...
      const orders = await request.get("/orders?isDeleted=false");
      dispatch(FetchSuccess(orders.data.data));
    } catch (error) {
...
export const FetchOrder = (id) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(FetchOrderSuccess(order.data.data));
    } catch (error) {
      dispatch(FetchError());
    }
  };
...
export const hookDeleteById = (id) => {
  return async (dispatch) => {
    try {
      dispatch(FetchRequest());
      const delOrd = await request.delete(`/orders/${id}`);
      dispatch(deleteById(delOrd.data.data));
      toast.success(
        DetectLang ? "تم مسح الطلب بنجاح" : "Order Deleted Successfully"
      );
    } catch (error) {
      dispatch(FetchError(error.message));
      toast.error(
