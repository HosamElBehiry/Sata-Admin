...
export const FetchBrands = () => {
  return async (dispatch) => {
    try {
      ...
      const brands = await request.get("/brands?isDeleted=false");
...
export const hookDelById = (id) => {
  return async (dispatch) => {
    try {
      dispatch(FetchRequest());
      const brdData = await request.delete(`/brands/${id}`);
      dispatch(deleteById(brdData.data.data));
      toast.success(flag ? "تم المسح بنجاح" : "Deleted Successfully");
    } catch (error) {
      dispatch(FetchError(error.message));
      toast.error(flag ? "خطا اثناء ارسال البيانات" : "Error From Server !");
    }
...
// اللى بتعمل سوفت ديليت لكذا علامه تحاريه
...
export const hookDeleteMany = (ids) => {
  return async (dispatch) => {
    try {
      dispatch(FetchRequest());
      const dltMany = await request.delete("/brands/many-brands", {
        data: ids,
      });
      dispatch(deleteMany(dltMany.data));
      toast.success(flag ? "تم المسح بنجاح" : "Deleted Successfully");
    } catch (error) {
      dispatch(FetchError(error.message));
      toast.error(flag ? "خطا اثناء ارسال البيانات" : "Error From Server !");
