...
const updateMany = (data) => {
  return {
    type: types.UPDATE_MANY,
    payload: data,
  };
};

export const hookUpdateMany = (ids) => {
  return async (dispatch) => {
    try {
      dispatch(FetchRequest());
      const dltCmps = await request.put("/shipping-company/many-comp", ids);
      dispatch(updateMany(dltCmps.data.data));
