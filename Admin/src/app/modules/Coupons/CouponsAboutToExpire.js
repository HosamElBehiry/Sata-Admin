...
} from "../../actions/coupons/coupons";
import { connect } from "react-redux";
import Moment from "react-moment";
import { FormattedMessage } from "react-intl";
function CouponsAboutToExpire({
  getAllCouponsAboutExpire,
  DeleteCouponById,
  coupons: { expire },
}) {
            ...
            expire.map((exp) => (
              <tr className="text-center border-3 m-auto" key={exp._id}>
                <td className="border text-center">
                  <div className="d-flex flex-column">{exp.code}</div>
                </td>
                <td className="border text-center">
                  <div className="d-flex flex-column">{exp.amount}</div>
                </td>
                <td className="border text-center">
                ...
                <td className="border text-center">
                  <span>
                    ...
                    <i
                      ...
                      onClick={(e) => DeleteCouponById(exp._id)}
                    />
                  </span>
                  <span></span>
                </td>
