...
import { hookDeleteById } from "../../../actions/orders/orderActions";
import { CancelRequest } from "../../shared/CancelRequest";
import Pagination from "../../shared/Pagination";
import DispLang from "../../../../utils/HEADERS";

const CardBody = ({ orders }) => {
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const data = useMemo(() => orders, [orders]);
  const columns = useMemo(() => {
    return user.roles[0] === 1 ? COLUMNS : COLUMN;
  }, [user]);
                        ...
                        {i === 0 ? (
                          ...
                          >
                            <div className="dropdown dropdown-inline">
                              <div
                                className={`dropdown dropdown-inline ${user
                                  .roles[0] === 2 && "d-none"}`}
                                onClick={() =>
                                  history.push(
                                    `/orders/order-item/${row.original._id}`
                                  )
                                }
                              >
                                <span
                            ...
                            <span
                              ...
                              onClick={() =>
                                `${
                                  user.roles[0] === 1
                                    ? history.push(
                                        `/orders/update-order/${row.original._id}`
                                      )
                                    : history.push(
                                        `/orders/update-order-item/${row.original._id}`
                                      )
                                }`
                              }
                            >
