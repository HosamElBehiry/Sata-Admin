...
// دى بتعمل فلتر للطلبات بناء على الحاله اذا كان معلق او مؤكد وغيره

const fetchByType = async ({ queryKey }) => {
  return await request({ url: `/orders/type/${queryKey[1]}` });
};

export const useFetchByType = (type) => {
  return useQuery(["get-by-type", type], fetchByType, {
