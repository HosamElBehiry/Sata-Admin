...
import { useAdminData } from "../shared/useAllData";
...
export function UserNotificationsDropdown() {
  ...
  const { data, isError, error } = useAdminData(user._id);
              ...
              <OverlayTrigger
                ...
                overlay={
                  <Tooltip id="user-notification-tooltip">
                    {DispLang ? `الاشعارات` : `Notifications`}
                  </Tooltip>
                }
              >
                <div
                    ...
                    <h4 className="d-flex flex-center rounded-top">
                      ...
                      <span className="btn btn-text btn-success btn-sm font-weight-bold btn-font-md ml-2">
                        {data?.data.data.Notifications.length > 0
                          ? DispLang
                            ? ` ${data?.data.data.Notifications.length} جديد`
                            : `${data?.data.data.Notifications.length} new`
                          : DispLang
                          ? `لا يوجد`
                          : " No Notifications"}
                      </span>
                      ...
                      {data?.data && data?.data.length !== 0 ? (
                        ...
                        >
                          <PerfectScrollbar
                            ...
                            style={{ maxHeight: "300px", position: "relative" }}
                          >
                            {data?.data.data.Notifications.map(
                              (notification) => {
                                return (
                                  <a
                                    href="#"
                        ...
                        >
                          <div className="d-flex flex-center text-center text-muted min-h-200px">
                            {DispLang ? `كل شئ على ما يرام` : `All caught up!`}
                            <br />
                            {DispLang
                              ? `لا يوجد اشعارات`
